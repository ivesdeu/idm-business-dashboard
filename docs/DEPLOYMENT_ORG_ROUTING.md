...
[`public/_headers`](../public/_headers) is copied next to `_redirects` in `dist/` and sets baseline security headers (frame denial, referrer, MIME sniffing, permissions). CSP is not sent until you add an enforcing **`Content-Security-Policy`** with a tested policy (report-only without a `report-to` endpoint only spams the console and has no effect). When you add CSP, Meeting Notes transcription needs `wss://streaming.assemblyai.com` and `https://streaming.assemblyai.com` in `connect-src`; `microphone=(self)` is already in Permissions-Policy. The app relies on **`'unsafe-inline'`** for large inline scripts/styles in `index.html`; removing that requires nonces or extracting bundles.
